...
app$1.whenReady().then(() => {
  ...
  ipcMain$1.handle("select-default-path", async () => {
  ...
  ipcMain$1.handle("select-folder-and-save", async (event, data) => {
    const defaultPath = store.get("defaultPath", app$1.getPath("documents"));
    const result = await dialog.showOpenDialog(mainWindow, {
      defaultPath,
      properties: ["openDirectory"],
    ...
    try {
      const content2 = JSON.stringify(data, null, 2);
      await fs$1.writeFile(filePath, content2, "utf-8");
